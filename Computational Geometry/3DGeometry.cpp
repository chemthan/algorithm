...
struct point_t {
    ...
    friend point_t det(point_t a, point_t b) {
        return point_t(a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z, a.x * b.y - a.y * b.x);
    }
    friend num_t mix(point_t a, point_t b, point_t c) {
        return a.x * b.y * c.z + a.y * b.z * c.x + a.z * b.x * c.y - a.z * b.y * c.x - a.x * b.z * c.y - a.y * b.x * c.z;
    }
    friend num_t dot(point_t a, point_t b) {
        return a.x * b.x + a.y * b.y + a.z * b.z;
    }
    num_t distLP(point_t p1, point_t p2) const {
        return det(p2 - p1, *this - p1).len() / (p2 - p1).len();
    }
    num_t distFP(point_t p1, point_t p2, point_t p3) const {
...
num_t distLL(point_t p1, point_t p2, point_t q1, point_t q2) {
...
num_t distSS(point_t p1, point_t p2, point_t q1, point_t q2) {
    point_t p = q1 - p1, u = p2 - p1, v = q2 - q1;
    num_t d = u.norm() * v.norm() - dot(u, v) * dot(u, v);
...
int isFL(point_t p, point_t o, point_t q1, point_t q2, point_t& res) {
    num_t a = dot(o, q2 - p), b = dot(o, q1 - p), d = a - b;
    if (sign(d) == 0) return 0;
    res = (q1 * a - q2 * b) / d;
    return 1;
}
int isFF(point_t p1, point_t o1, point_t p2, point_t o2, point_t& a, point_t& b) {
...
num_t volume(point_t a, point_t b, point_t c, point_t d) {
    return nabs(mix(b - a, c - a, d - a) / 6);
}

int main() {
    srand(time(NULL));
    point_t a(rand() % 1000, rand() % 1000, rand() % 1000);
    point_t b(rand() % 1000, rand() % 1000, rand() % 1000);
    point_t c(rand() % 1000, rand() % 1000, rand() % 1000);
    point_t d(rand() % 1000, rand() % 1000, rand() % 1000);
    cout << fixed << setprecision(9) << (long double) volume(a, b, c, d) << "\n";
    cout << fixed << setprecision(9) << (long double) nabs(dot(det(b - a, c - a), d - a)) / 6 << "\n";
    return 0;
}
