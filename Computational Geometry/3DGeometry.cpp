...
template <class num_t> struct point_t {
  ...
  inline num_t len() { return hypot(hypot(x, y), z); }
  inline num_t norm() { return x * x + y * y + z * z; }
  ...
  inline friend point_t det(point_t a, point_t b) {
    return point_t(a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z,
                   a.x * b.y - a.y * b.x);
  }
  ...
  friend num_t distSS(point_t p1, point_t p2, point_t q1, point_t q2) {
    ...
    num_t d = u.norm() * v.norm() - dot(u, v) * dot(u, v);
    if (sign(d) == 0)
      return min(min((p1 - q1).len(), (p1 - q2).len()),
  ...
  friend int isFF(point_t p1, point_t o1, point_t p2, point_t o2, point_t &a,
                  point_t &b) {
