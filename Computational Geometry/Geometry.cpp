...
// Determine if line segment from a to b intersects with line segment from c to
// d
...
int PointInPolygon(vector<point_t> &p, point_t q) {
  ...
  if (Det(p[0], p[a], p[b]) > 0)
    swap(a, b);
  // Allow on edge --> if (Det... > 0 || Det ... < 0)
  if (Det(p[0], p[a], q) >= 0 || Det(p[0], p[b], q) <= 0)
    return 0;
  while (abs(a - b) > 1) {
    c = (a + b) / 2;
    if (Det(p[0], p[c], q) > 0)
...
// Compute intersection of line through points a and b with circle centered at c
// with radius r > 0
...
int main() {
  ...
  // expected: (5,2)
  cout << ProjectPointLine(point_t(-5, -2), point_t(10, 4), point_t(3, 7))
       << endl;
  ...
  cout << ProjectPointSegment(point_t(-5, -2), point_t(10, 4), point_t(3, 7))
       ...
       << " "
       << ProjectPointSegment(point_t(-5, -2), point_t(2.5, 1), point_t(3, 7))
       << endl;
  ...
  // expected: 1 0 1
  cout << LinesParallel(point_t(1, 1), point_t(3, 5), point_t(2, 1),
                        point_t(4, 5))
       << " "
       << LinesParallel(point_t(1, 1), point_t(3, 5), point_t(2, 0),
                        point_t(4, 5))
       << " "
       << LinesParallel(point_t(1, 1), point_t(3, 5), point_t(5, 9),
                        point_t(7, 13))
       << endl;

  // expected: 0 0 1
  cout << LinesCollinear(point_t(1, 1), point_t(3, 5), point_t(2, 1),
                         point_t(4, 5))
       << " "
       << LinesCollinear(point_t(1, 1), point_t(3, 5), point_t(2, 0),
                         point_t(4, 5))
       << " "
       << LinesCollinear(point_t(1, 1), point_t(3, 5), point_t(5, 9),
                         point_t(7, 13))
       << endl;

  // expected: 1 1 1 0
  cout << SegmentsIntersect(point_t(0, 0), point_t(2, 4), point_t(3, 1),
                            point_t(-1, 3))
       << " "
       << SegmentsIntersect(point_t(0, 0), point_t(2, 4), point_t(4, 3),
                            point_t(0, 5))
       << " "
       << SegmentsIntersect(point_t(0, 0), point_t(2, 4), point_t(2, -1),
                            point_t(-2, 1))
       << " "
       << SegmentsIntersect(point_t(0, 0), point_t(2, 4), point_t(5, 5),
                            point_t(1, 7))
       << endl;

  // expected: (1,2)
  cout << ComputeLineIntersection(point_t(0, 0), point_t(2, 4), point_t(3, 1),
                                  point_t(-1, 3))
       << endl;

  // expected: (1,1)
  cout << ComputeCircleCenter(point_t(-3, 4), point_t(6, 1), point_t(4, 5))
       << endl;
  ...
  //           (4,5) (5,4)
  vector<point_t> u =
      CircleLineIntersection(point_t(0, 6), point_t(2, 6), point_t(1, 1), 5);
  ...
  for (int i = 0; i < u.size(); i++)
    cerr << u[i] << " ";
  cout << endl;
  u = CircleCircleIntersection(point_t(1, 1), point_t(4.5, 4.5), 10,
                               sqrt(2.0) / 2.0);
  for (int i = 0; i < u.size(); i++)
    cerr << u[i] << " ";
  cout << endl;
  u = CircleCircleIntersection(point_t(1, 1), point_t(4.5, 4.5), 5,
                               sqrt(2.0) / 2.0);
  for (int i = 0; i < u.size(); i++)
    cerr << u[i] << " ";
  cout << endl;
