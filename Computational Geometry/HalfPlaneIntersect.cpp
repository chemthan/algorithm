...
int main() {
  ...
  qh = qe = 0;
  for (int i = 0; i < vpl.size(); i++) {
    while (qe - qh > 1 && !check(vpl[i], que[qe - 2], que[qe - 1]))
      qe--;
    while (qe - qh > 1 && !check(vpl[i], que[qh], que[qh + 1]))
      qh++;
    que[qe++] = vpl[i];
  }
  while (qe - qh > 2 && !check(que[qh], que[qe - 2], que[qe - 1]))
    qe--;
  while (qe - qh > 2 && !check(que[qe - 1], que[qh], que[qh + 1]))
    qh++;
  return 0;
}
