...
struct point_t {
  ...
  int operator<(const point_t &rhs) const {
    return make_pair(x, make_pair(y, idx)) <
           make_pair(rhs.x, make_pair(rhs.y, rhs.idx));
  }
};
...
int dist(point_t a, point_t b) { return abs(a.x - b.x) + abs(a.y - b.y); }

void add(int u, int v, int d) { pq.push(make_pair(d, make_pair(u, v))); }
...
void manhattan() {
  ...
  for (int dir = 0; dir < 4; dir++) {
    ...
    for (int i = n - 1; i >= 0; i--) {
      int pos = query(a[i]);
      if (~pos)
        add(p[i].idx, p[pos].idx, dist(p[i], p[pos]));
