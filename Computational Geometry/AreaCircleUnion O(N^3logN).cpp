...
vector<point_t> CircleCircleIntersection(point_t a, point_t b, double r,
  ...
  if (d > r + R - EPS || d + min(r, R) < max(r, R) + EPS)
    return res;
...
double calc(double lo, double hi, vector<Circle> &vc) {
  ...
  for (int i = 0; i < vc.size(); i++) {
    ...
    if (!sign(lo, vc[i].c.y - vc[i].r)) {
      ...
      vl.push_back(vc[i].c.x);
    } else if (sign(lo, vc[i].c.y - vc[i].r) > 0 &&
               sign(lo, vc[i].c.y + vc[i].r) < 0) {
      vector<point_t> res =
          CircleLineIntersection(point_t(0, lo), point_t(1, lo),
                                 point_t(vc[i].c.x, vc[i].c.y), vc[i].r);
      if (res.size() == 2) {
        if (res[0].x > res[1].x) {
          swap(res[0], res[1]);
        }
    ...
    if (!sign(hi, vc[i].c.y + vc[i].r)) {
      ...
      vr.push_back(vc[i].c.x);
    } else if (sign(hi, vc[i].c.y - vc[i].r) > 0 &&
               sign(hi, vc[i].c.y + vc[i].r) < 0) {
      vector<point_t> res =
          CircleLineIntersection(point_t(0, hi), point_t(1, hi),
                                 point_t(vc[i].c.x, vc[i].c.y), vc[i].r);
      if (res.size() == 2) {
        if (res[0].x > res[1].x) {
          swap(res[0], res[1]);
    ...
    if (vl.size() == 2 && vr.size() == 2) {
      for (int j = 0; j < 2; j++) {
        vx.push_back(make_pair(
            make_pair((vl[j] + vr[j]) / 2, j),
            make_pair(make_pair(point_t(vl[j], lo), point_t(vr[j], hi)), i)));
      }
    }
  }
  ...
  for (int i = 0; i < vx.size(); i++) {
    ...
    if (!cnt) {
      ...
      res += Area(vc[ix2].c, vc[ix2].r, vx[i].second.first.first,
                  vx[i].second.first.second);
      res += (fabs(cur.first.first.x - vx[i].second.first.first.x) +
              fabs(cur.first.second.x - vx[i].second.first.second.x)) *
             (hi - lo) / 2;
    }
  }
  return res;
...
double calc(vector<Circle> vc) {
  ...
  for (int i = 0; i < vc.size(); i++) {
    ...
    for (int j = i + 1; j < vc.size(); j++) {
      vector<point_t> res =
          CircleCircleIntersection(vc[i].c, vc[j].c, vc[i].r, vc[j].r);
