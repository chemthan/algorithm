...
struct Edge {
  ...
  Cost cost;
  Edge() {}
  Edge(int id, int u, int v, num_t c) : u(u), v(v) { cost = Cost(id, c, 0); }
} edge[maxe];
...
void add(int id, int u, int v, num_t c) { edge[m++] = Edge(id, u, v, c); }
num_t mst(int root) {
  num_t res = 0;
  while (1) {
    for (int i = 0; i < n; i++)
    ...
    for (int e = 0; e < m; e++) {
      ...
      if (edge[e].u != edge[e].v)
        edge[e].cost = Cost(edge[e].cost.pos, best[v]);
    }
    n = cntnode;
    root = node[root];
  }
