...
template <class T> struct CascadingSegment {
  struct Info {
    ...
    int operator<(const Info &rhs) const { return key < rhs.key; }
  };
  ...
  void upd(int p, int i, int L, int R, T val) {
    ...
    if (L < R) {
      ...
      while (lptr < st[p << 1].size() && val >= st[p << 1][lptr].key)
        lptr++;
      while (rptr < st[p << 1 | 1].size() && val >= st[p << 1 | 1][rptr].key)
        rptr++;
    }
  ...
  int query(int p, int l, int r, int L, int R, T val) {
    if (l > R || r < L)
      return 0;
    if (p == 1) {
      int k = upper_bound(st[p].begin(), st[p].end(), Info(val)) -
              st[p].begin() - 1;
      if (!~k)
        return 0;
      if (l <= L && r >= R)
        return k + 1;
      return query(p << 1, l, r, L, L + R >> 1, st[p][k].lnxt - 1) +
             query(p << 1 | 1, l, r, (L + R >> 1) + 1, R, st[p][k].rnxt - 1);
    }
    if (!~val)
      return 0;
    if (l <= L && r >= R)
      return val + 1;
    return query(p << 1, l, r, L, L + R >> 1, st[p][val].lnxt - 1) +
           query(p << 1 | 1, l, r, (L + R >> 1) + 1, R, st[p][val].rnxt - 1);
  }
};
