...
long long calc(vector<tuple<num_t, num_t, num_t, num_t>>& recs) {
    ...
    function<void(int, int, int, int, int, int)> upd = [&] (int p, int l, int r, int L, int R, int val) {
        ...
        if (l <= L && R <= r) {
            ...
            else if (L < R) st[p] = st[p << 1] + st[p << 1 | 1];
            else st[p] = 0;
            return;
        }
    ...
    vector<tuple<long long, int, int, int>> vals;
    for (auto& rec : recs) {
    ...
    for (auto e : vals) {
        long long x; int s, lo, hi; tie(x, s, lo, hi) = e;
        res += st[1] * (x - cur);
