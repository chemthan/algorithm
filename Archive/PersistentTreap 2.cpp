...
void pushup(Node *a) { a->size = a->l->size + a->r->size + 1; }
void init() { nil->prio = ~0U >> 1; }
int randprio() {
  static int seed = 0;
...
int findpos(Node *root, int key) {
  if (root == nil)
    return 0;
  if (root->key > key)
    return findpos(root->l, key);
  if (root->key < key)
    return root->l->size + 1 + findpos(root->r, key);
  if (root->key = key)
...
struct PersistentTreap {
  Node *root;
  PersistentTreap() { root = nil; }
  Node *splitL(Node *a, int size) {
    if (a == nil || !size)
      return nil;
    if (a->l->size >= size)
      return splitL(a->l, size);
    if (a->l->size + 1 >= size)
      return alloc(a->key, a->l, nil, a->prio);
  ...
  Node *splitR(Node *a, int size) {
    if (a == nil || !size)
      return nil;
    if (a->r->size >= size)
      return splitR(a->r, size);
    if (a->r->size + 1 >= size)
      return alloc(a->key, nil, a->r, a->prio);
  ...
  Node *merge(Node *a, Node *b) {
    ...
    if (b == nil)
      return a;
    if (a->prio > b->prio)
      return alloc(a->key, a->l, merge(a->r, b), a->prio);
    return root = alloc(b->key, merge(a, b->l), b->r, b->prio);
  }
  Node *insert(int key, int p) {
    int l = root->size;
    return root = merge(merge(splitL(root, p), alloc(key, nil, nil)),
                        splitR(root, l - p));
  }
  Node *insert(int key) {
    return root = merge(merge(splitLkey(root, key), alloc(key, nil, nil)),
                        splitRkey(root, key));
  }
