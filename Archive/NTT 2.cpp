...
int init(int n) {
  ...
  assert(k <= 19);
  p = 7 * 17;
  for (int i = 1; i <= 23 - k; i++)
    p *= 2;
  E1 = fpow(3, p, P1);
  F1 = fpow(E1, P1 - 2, P1);
  I1 = fpow(N, P1 - 2, P1);
  p = 9 * 211;
  for (int i = 1; i <= 19 - k; i++)
    p *= 2;
  E2 = fpow(5, p, P2);
  F2 = fpow(E2, P2 - 2, P2);
  I2 = fpow(N, P2 - 2, P2);
...
void mul(int *a, int *b, int *c, int n, int p) {
  int N = init(n);
  memcpy(X, a, sizeof(*a) * n);
  memcpy(Y, b, sizeof(*b) * n);
  memset(A, 0, sizeof(*A) * N);
  memset(B, 0, sizeof(*B) * N);
  memset(C, 0, sizeof(*C) * N);
  memset(c, 0, sizeof(*c) * N);
  memcpy(A, X, sizeof(*X) * n);
  memcpy(B, Y, sizeof(*Y) * n);
  NTT(A, P1, E1, N);
  NTT(B, P1, E1, N);
  for (int i = 0; i < N; i++)
    C[i] = (long long)A[i] * B[i] % P1;
  NTT(C, P1, F1, N);
  for (int i = 0; i < N; i++)
    C[i] = (long long)C[i] * I1 % P1;
  NTT(X, P2, E2, N);
  NTT(Y, P2, E2, N);
  for (int i = 0; i < N; i++)
    c[i] = (long long)X[i] * Y[i] % P2;
  NTT(c, P2, F2, N);
  for (int i = 0; i < N; i++)
    c[i] = (long long)c[i] * I2 % P2;
  for (int i = 0; i < N; i++)
    c[i] = CRT(C[i], c[i], p);
  for (int i = n; i < N; i++)
    c[i] = 0;
