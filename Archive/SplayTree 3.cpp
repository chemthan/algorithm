...
int isrt(Node *x) { return x->p == nil || (x->p->l != x && x->p->r != x); }
...
void pushup(Node *x) { x->size = x->l->size + x->r->size + 1; }
void rotate(Node *x) {
  Node *y = x->p;
  int l = x->p->l == x;
  if (!isrt(y)) {
...
void splay(Node *x) {
  ...
  while (!isrt(x)) {
    ...
    pushdown(x);
    if (!isrt(x->p))
      rotate((x->p->l == x) == (x->p->p->l == x->p) ? x->p : x);
    rotate(x);
  }
  pushup(x);
}
...
struct SplayTree {
  ...
  Node *findkey(int key) { return findkey(root, key); }
  ...
  Node *findpos(int pos) { return findpos(root, pos); }
