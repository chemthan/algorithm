 ...
 * Sufficient condition: w[a][c] + w[b][d] <= w[b][c] + w[a][d], a <= b <= c <=
 * d Problem:
...
inline num_t lookup(int j, int i) {
  if (!i)
    return g[j + c];
  return B(i + r - 1, j + c);
}
...
void SMAWK(int n, int inc, int ncol, int row_minima[]) {
  ...
  for (int i = 0; i < ncol; i++) {
    while (num && lookup(inc * (num - 1), sub_col[num - 1]) >=
                      lookup(inc * (num - 1), col[i]))
      num--;
    if (num < row_size)
      sub_col[num++] = col[i];
  }
