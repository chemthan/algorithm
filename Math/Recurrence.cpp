...
/*
 * Recurrence formula: x[i] = c[k - 1] * x[i - 1] + c[k - 2] * x[i - 2] + ... +
 * c[0] * x[i - k] x[0], x[1],..., x[2 * k - 1] are given Return c[0], c[1],...,
 * c[k - 1]
 */
struct InvertRecurrence {
  ...
  void gauss() {
    ...
    for (int i = 0; i < n - 1; i++) {
      ...
      for (int j = i + 1; j < n; j++) {
        for (int k = i + 1; k < m; k++) {
          a[j][k] = ((long long)a[j][k] * a[i][i] % MOD -
                     (long long)a[j][i] * a[i][k] % MOD + MOD) %
                    MOD;
        }
        a[j][i] = 0;
      }
...
int main() {
  ...
  for (int i = ir.k; i < (ir.k << 1); i++) {
    for (int j = 0; j < ir.k; j++) {
      ir.x[i] =
          (ir.x[i] + (long long)ir.c[ir.k - j - 1] * ir.x[i - j - 1]) % MOD;
    }
  }
  for (int i = 0; i < ir.k; i++) {
    cout << ir.c[i] << " \n"[i == ir.k - 1];
