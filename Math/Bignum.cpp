...
void multiply(int a[], int b[], int na, int nb, int c[], int &nc, int base,
  ...
  for (int i = 0; i < n; i++) {
    ...
    long long ncarry = (u / base << 15) + (u % base << 15) / base + v / base +
                       (w / base << 30) + (w % base << 30) / base;
    assert(0 <= ncarry && ncarry < 2e18);
    long long t =
        (u % base << 15) % base + v % base + (w % base << 30) % base + carry;
    carry = ncarry + t / base;
    assert(carry < 2e18);
    c[nc++] = t % base;
  }
...
struct Bignum {
  ...
  Bignum operator/(const Bignum &v) const { return divmod(*this, v).first; }
  Bignum operator%(const Bignum &v) const { return divmod(*this, v).second; }
  void operator/=(int v) {
    if (v < 0)
      sign = -sign, v = -v;
  ...
  void operator/=(long long v) { *this /= Bignum(v); }
  Bignum operator/(int v) const {
    Bignum res = *this;
    res /= v;
    return res;
  ...
  bool operator!=(const Bignum &v) const { return *this < v || v < *this; }
  void trim() {
  ...
  void read(const string &s) {
    ...
    for (int i = s.size() - 1; i >= pos; i -= nblock) {
      int x = 0;
      for (int j = max(pos, i - nblock + 1); j <= i; j++)
        x = x * base + s[j] - '0';
      a.push_back(x);
    }
    trim();
  ...
  friend ostream &operator<<(ostream &stream, const Bignum &v) {
    ...
    for (int i = (int)v.a.size() - 2; i >= 0; i--)
      stream << setw(nblock) << setfill('0') << v.a[i];
    return stream;
  }
  ...
  static vector<long long> karatsuba(vector<long long> &a,
                                     vector<long long> &b) {
    int n = a.size();
  ...
  friend Bignum pow(Bignum a, int b) { return pow(a, (Bignum(b))); }
