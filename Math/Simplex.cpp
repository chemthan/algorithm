 ...
 */
template <typename num_t> struct LPsolver {
  ...
  LPsolver(const vector<vector<double>> &A, const vector<num_t> &b,
           const vector<num_t> &c)
      : m(b.size()), n(c.size()), N(n + 1), B(m),
        D(m + 2, vector<num_t>(n + 2)) {
    for (int i = 0; i < m; i++)
      for (int j = 0; j < n; j++)
        D[i][j] = A[i][j];
    for (int i = 0; i < m; i++) {
  ...
  num_t solve(vector<num_t> &x) {
    ...
    if (sign(D[r][n + 1]) < 0) {
      pivot(r, n);
      if (!simple(1) || sign(D[m + 1][n + 1]) < 0)
        return -numeric_limits<num_t>::infinity();
      for (int i = 0; i < m; i++)
        if (B[i] == -1) {
          int s = -1;
          for (int j = 0; j <= n; j++) {
