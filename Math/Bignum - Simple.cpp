...
Bignum operator+(Bignum a, Bignum b) {
  if (a.len() < b.len())
    swap(a, b);
  Bignum c;
  c.data = a.data, c.data.push_back(0);
  int r = 0;
  for (int i = 0; i < c.len(); i++) {
    c[i] += ((i < b.len()) ? b[i] : 0) + r;
    if ((r = (c[i] >= Bignum::BASE)))
      c[i] -= Bignum::BASE;
  }
  c.trim();
  return c;
...
Bignum operator*(Bignum a, int m) {
  ...
  while (r)
    a.data.push_back(r % Bignum::BASE), r /= Bignum::BASE;
  a.trim();
  return a;
}
...
Bignum operator/(Bignum a, int m) { return divmod(a, m).first; }
int operator%(Bignum a, int m) { return divmod(a, m).second; }
Bignum operator*(const Bignum &a, const Bignum &b) {
  ...
  for (int i = 0; i < a.data.size(); i++) {
    ...
    for (int j = 0; j < b.data.size() || carry > 0; j++) {
      long long s =
          res[i + j] + carry +
          (long long)a.data[i] * (j < b.data.size() ? (long long)b.data[j] : 0);
...
ostream &operator<<(ostream &stream, const Bignum &a) {
  ...
  for (int i = (int)a.data.size() - 2; i >= 0; i--)
    stream << setw(Bignum::NBLOCK) << setfill('0') << a.data[i];
  return stream;
}
