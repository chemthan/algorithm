...
template <class num_t> inline num_t mult(num_t a, num_t b, num_t p) {
...
template <class num_t> inline num_t fpow(num_t n, num_t k, num_t p) {
  num_t r = 1;
  for (; k; k >>= 1) {
    if (k & 1)
      r = mult(r, n, p);
...
template <class num_t> inline num_t isqrt(num_t k) {
  num_t r = sqrt(k) + 1;
  while (r * r > k)
    r--;
  return r;
