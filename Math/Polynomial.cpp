...
struct Polynomial {
  ...
  int deg;
  vector<int> cof;
  Polynomial(int deg = 0) : deg(deg) { cof.resize(deg + 1, 0); }
  void init(int deg) {
    this->deg = deg;
    cof.resize(deg + 1, 0);
    for (int i = 0; i <= deg; i++) {
  ...
  friend Polynomial quotient(Polynomial f, Polynomial g) {
    ...
    for (int i = q.deg; i >= 0; i--) {
      q.cof[i] =
          (long long)f.cof[g.deg + i] * fpow(g.cof[g.deg], mod - 2, mod) % mod;
      for (int j = g.deg; j >= 0; j--) {
        f.cof[i + j] =
            (f.cof[i + j] - (long long)q.cof[i] * g.cof[j] % mod + mod) % mod;
      }
    }
    return q;
  }
  friend Polynomial remainder(Polynomial f, Polynomial g) {
    ...
    for (int i = f.deg - g.deg; i >= 0; i--) {
      ...
      for (int j = g.deg; j >= 0; j--) {
        f.cof[i + j] =
            (f.cof[i + j] - (long long)c * g.cof[j] % mod + mod) % mod;
      }
    }
    f.deg = g.deg - 1;
    while (!f.cof[f.deg])
  ...
  friend vector<int> findroots(Polynomial f) {
    ...
    if (f.deg == 1) {
      int root =
          (mod - (long long)f.cof[0] * fpow(f.cof[1], mod - 2, mod) % mod) %
          mod;
      res.push_back(root);
      return res;
    }
