 ...
 * Matrix a[][] is totally monotone iff for any 0 <= i1 < i2 < n, 0 <= j1 < j2 <
 * m a[i2][j1] < a[i2][j2] => a[i1][j1] < a[i1][j2] a[i2][j1] = a[i2][j2] =>
 * a[i1][j1] <= a[i1][j2]
 */

int a[9][18] = {
    {25, 21, 13, 10, 20, 13, 19, 35, 37, 41, 58, 66, 82, 99, 124, 133, 156,
     178},
    {42, 35, 26, 20, 29, 21, 25, 37, 36, 39, 56, 64, 76, 91, 116, 125, 146,
     164},
    {57, 48, 35, 28, 33, 24, 28, 40, 37, 37, 54, 61, 72, 83, 107, 113, 131,
     146},
    {78, 65, 51, 42, 44, 35, 38, 48, 42, 42, 55, 61, 70, 80, 100, 106, 120,
     135},
    ...
    {151, 130, 104, 88, 80, 59, 52, 49, 37, 29, 29, 24, 23, 20, 28, 25, 31,
     39}};

int lookup(int i, int j) { return a[i][j]; }
...
void SMAWK(int n, int inc, vector<int> col, int row_minima[]) {
  ...
  for (int i = 0; i < col.size(); i++) {
    while (sub_col.size() &&
           lookup(inc * (sub_col.size() - 1), sub_col.back()) >=
               lookup(inc * (sub_col.size() - 1), col[i]))
      sub_col.pop_back();
    if (sub_col.size() < row_size)
      sub_col.push_back(col[i]);
  }
  col = sub_col;
  ...
  for (int i = inc, c = 0; i < n; i += 2 * inc) {
    ...
    while (c < col.size() && col[c] <= next) {
      if (lookup(i, col[c]) <= lookup(i, res))
        res = col[c];
      c++;
    }
    c--;
