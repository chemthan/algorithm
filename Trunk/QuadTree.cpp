...
void upd(int px, int py, int l1, int r1, int l2, int r2, int L1, int R1, int L2,
         int R2, int val) {
  ...
  if (L1 < R1 && L2 < R2) {
    ...
    if (L2 < R2)
      npy = rnode(py);
    upd(npx, npy, l1, r1, l2, r2, mid(L1, R1) + 1, R1, mid(L2, R2) + 1, R2,
        val);
    mx = max(mx, st[npx][npy]);
  }
  if (mx)
...
int query(int px, int py, int l1, int r1, int l2, int r2, int L1, int R1,
          int L2, int R2) {
  pushdown(px, py, L1, R1, L2, R2);
  if (!insect(L1, R1, L2, R2, l1, r1, l2, r2))
    return 0;
  ...
  if (L1 < R1 || L2 < R2) {
    ...
    if (L2 < R2)
      npy = lnode(py);
    res = max(
        res, query(npx, npy, l1, r1, l2, r2, L1, mid(L1, R1), L2, mid(L2, R2)));
  }
  if (L2 < R2) {
    int npx = px, npy = py;
    if (L1 < R1)
    ...
    if (L2 < R2)
      npy = rnode(py);
    res = max(res, query(npx, npy, l1, r1, l2, r2, L1, mid(L1, R1),
                         mid(L2, R2) + 1, R2));
  }
  if (L1 < R1) {
    int npx = px, npy = py;
    ...
    if (L2 < R2)
      npy = lnode(py);
    res = max(res, query(npx, npy, l1, r1, l2, r2, mid(L1, R1) + 1, R1, L2,
                         mid(L2, R2)));
  }
  if (L1 < R1 && L2 < R2) {
    int npx = px, npy = py;
    ...
    if (L2 < R2)
      npy = rnode(py);
    res = max(res, query(npx, npy, l1, r1, l2, r2, mid(L1, R1) + 1, R1,
                         mid(L2, R2) + 1, R2));
  }
  return res;
}
