...
struct Circle {
  ...
  int operator<(const Circle &rhs) const { return rad < rhs.rad; }
};
...
Circle Enclose(vector<PT> &vp, PT p, PT q) {
  ...
  for (int i = 0; i < vp.size(); i++)
    if (rlcmp(cross(p - vp[i], q - vp[i])) >= 0) {
      Circle c = CircumCircle(p, q, vp[i]);
      RL d = DistancePointLine(p, q, c.cen);
      if (rlcmp(cross(p - c.cen, q - c.cen)) < 0)
        d *= -1;
...
Circle Enclose(vector<PT> &vp) {
  ...
  vector<PT> cur;
  cur.push_back(vp[0]), cur.push_back(vp[1]);
